...
def addCategory(request, form_class=BudgetCategory,
                template="addCategory.HTML"):  # menambahkan suatu jenis budget (nama, sifat-sifat lainnya misalnya batasan nominal)
...
def deleteCategory(request, slug, form_class=BudgetCategory,
                   template="deleteCategory.HTML"):  # menghapus sebuah atau beberapa budget beserta seluruh budget yang tipenya dihapus
...
def cancelOperation(request):  # membatalkan operasi method apapun, dan mengembalikannya ke halaman utama
    return redirect(request, "budgetcategory")
